...
impl CustomApiSource {
    ...
    async fn get_json(&self, url: &str) -> Result<serde_json::Value> {
        ...
        let resp = resp.error_for_status().context("custom source HTTP error")?;
        let text = resp.text().await.context("custom source response body")?;
        serde_json::from_str(&text).context("parse custom source JSON")
    }
}
