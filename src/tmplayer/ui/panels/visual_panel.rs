...
fn current_two_lines(app: &AppState) -> (&str, &str) {
    ...
    let translation = lines
        .get(idx)
        .and_then(|l| l.translation.as_deref())
        .filter(|t| !t.trim().is_empty());
    let l2 = if app.config.show_lyric_translation {
        translation
            .unwrap_or_else(|| lines.get(idx + 1).map(|l| l.text.as_str()).unwrap_or(""))
    } else {
        lines.get(idx + 1).map(|l| l.text.as_str()).unwrap_or("")
    };
    (l1, l2)
}
